...
final class DeviceTokenManager {
    ...
    func saveDeviceToken(_ token: String) {
        UserDefaults.standard.set(token, forKey: UserDefaultsKey.deviceToken.rawValue)
        Log.debug("디바이스 토큰 저장 완료: \(token)")
    }

    /// 디바이스 토큰이 있고 visitorId/artistId가 있으면 서버로 전송
    func registerDeviceTokenIfNeeded() {
        if UserDefaults.standard.bool(forKey: UserDefaultsKey.hasRegisteredDeviceToken.rawValue) {
            Log.debug("디바이스 토큰 이미 등록됨")
            return
        }

        // UserDefaults에서 저장된 디바이스 토큰 가져오기
        guard let token = UserDefaults.standard.string(forKey: UserDefaultsKey.deviceToken.rawValue)
        else {
        ...
        notificationService.registerDeviceToken(dto: dto) { result in
            ...
            case .success:
                ...
                UserDefaults.standard.set(
                    true, forKey: UserDefaultsKey.hasRegisteredDeviceToken.rawValue)
