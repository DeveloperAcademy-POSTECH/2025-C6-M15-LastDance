...
final class ExhibitionDetailViewModel: ObservableObject {
    ...
    // private let artistAPIService: ArtistAPIServiceProtocol // Removed

    private var currentArtistId: Int? // To store the ID of the current artist

    init(
        // artistAPIService: ArtistAPIServiceProtocol = ArtistAPIService() // Removed
    ) {
    ...
    private func fetchCurrentArtistId() {
        ...
        // Query SwiftData for the Artist object with this UUID
        let allArtists = dataManager.fetchAll(Artist.self)
        if let localArtistProfile = allArtists.first(where: { $0.uuid == artistUUID }) {
            self.currentArtistId = localArtistProfile.id
            Log.debug("ExhibitionDetailViewModel: Current artist ID fetched from SwiftData: \(localArtistProfile.id)")
        } else {
            Log.error("ExhibitionDetailViewModel: Local Artist profile with UUID \(artistUUID) not found in SwiftData.")
        }
    }
